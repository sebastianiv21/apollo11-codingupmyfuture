...
def generar_reportes(directorio: str, output_directory: str) -> None:
    ...
    try:
        ...
        conteo_eventos_mision = (
            df_total.groupby(["mission", "device_type"])
            .size()
            .reset_index(name="count")
        )
        conteo_eventos_mision = (
            conteo_eventos_mision.groupby(["mission", "device_type"])
            .agg({"count": "sum"})
            .reset_index()
        )
        ...
        max_unknown = (
            df_total[df_total["device_status"] == "unknown"]
            .groupby(["mission", "device_type"])
            .size()
            .reset_index(name="count")
        )
        dispositivo_max_unknown = max_unknown.loc[
            max_unknown.groupby("mission")["count"].idxmax()
        ]
        ...
        total_killed = (
            dispositivos_killed.groupby(["mission", "device_type"])
            .size()
            .reset_index(name="count")
        )

        # Guardar el resultado en un archivo YAML
        archivo_salida_3 = os.path.join(
            output_directory, "3_dispositivos_inoperables.log"
        )
        with open(archivo_salida_3, "w") as outfile:
            ...
            yaml.dump(
                {"total_inoperables": total_inoperables},
                outfile,
        ...
        porcentajes = (
            df_total.groupby(["mission", "device_type"]).size() / len(df_total) * 100
        )
